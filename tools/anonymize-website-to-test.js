...
function replacePrismicImages(html) {
  ...
  const prismicRe = /https:\/\/(images\.prismic\.io|l-one\.cdn\.prismic\.io)\/l-one\/[^\s"')>]+/g;
...
function replaceImgTags(html) {
  ...
  const replaced = html.replace(imgRe, (full, quote, src) => {
    ...
    count += 1;
    return full.replace(new RegExp(`\\bsrc=${quote}${src.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}${quote}`, "i"), `src=${quote}${picsum}${quote}`);
  });
...
function replaceBranding(html) {
  ...
  out = out.replace(/<!--\s*saved from url=\([^)]*\)[^>]*-->/i, "<!-- anonymized copy for accessibility workshop -->");

  // Title
  out = out.replace(/<title>[^<]*<\/title>/i, "<title>C-Company - Die Outsourcing-Experten für Softwareentwicklung</title>");
  ...
  out = out
    .replace(/https:\/\/www\.l-one\.de\b\/?/g, "https://www.c-company.example/")
    .replace(/https?:\/\/calendly\.com\/l-one-systems\//g, "https://calendly.com/c-company/");

  // Cleanup common leftover mail/links.
  out = out
    .replace(/mailto:info@l-one\.de/gi, "mailto:info@c-company.example")
    .replace(/https?:\/\/l-one\.systems\//gi, "https://www.c-company.example/")
  ...
  out = out
    ...
    .replace(/https?:\/\/c-company\.systems\//gi, "https://www.c-company.example/");

  // Avoid L-One slugs in URL paths.
  out = out.replace(/\/(l-one)-/g, "/c-company-");
  out = out.replace(/kennenlernen-l-one-[a-z0-9-]+/gi, "kennenlernen-c-company");

  // De-brand clutch profile link if present.
  out = out.replace(/https?:\/\/clutch\.co\/profile\/l-one-systems-gmbh#reviews/gi, "https://www.c-company.example/reviews");
...
function normalizeHeadMeta(html) {
  ...
  out = out.replace(/\bproperty=("|')og:description\1\1/gi, 'property="og:description"');
  ...
  if (!/\bproperty=("|')og:image\1/i.test(out)) {
    out = out.replace(/<head[^>]*>/i, (m) => `${m}<meta property="og:image" content="${ogImage}">`);
  } else {
    out = out.replace(/<meta\s+[^>]*\bproperty=("|')og:image\1\b[^>]*>/gi, `<meta property="og:image" content="${ogImage}">`);
  }
  ...
  if (!/\bname=("|')twitter:image\1/i.test(out)) {
    out = out.replace(/<head[^>]*>/i, (m) => `${m}<meta name="twitter:image" content="${twImage}">`);
  } else {
    out = out.replace(/<meta\s+[^>]*\bname=("|')twitter:image\1\b[^>]*>/gi, `<meta name="twitter:image" content="${twImage}">`);
  }
  ...
  out = out.replace(/<link\s+rel=canonical[^>]*>/gi, `<link rel="canonical" href="${canonicalHref}">`);
...
function ensureMaterialIconsLink(html) {
  if (/fonts\.googleapis\.com\/icon\?family=Material\+Icons/i.test(html)) return html;
...
function replaceCommonInlineSvgs(html) {
  ...
  out = out.replace(/<svg\b([^>]*\bviewBox=("|')0 0 20 20\2[^>]*)>[\s\S]*?<\/svg>/gi, (_full, attrs) => {
    count += 1;
  ...
  out = out.replace(/<svg\b([^>]*\bviewBox=("|')0 0 512 512\2[^>]*)>[\s\S]*?<\/svg>/gi, (_full, attrs) => {
    count += 1;
    const classMatch = attrs.match(/\bclass=("|')([^"']+)(\1)/i);
    const cls = classMatch ? classMatch[2] : "";
    const mergedClass = cls ? `material-icons ${cls}` : "material-icons";
...
function main() {
  ...
  if (!inputPath) {
    console.error("Usage: node tools/anonymize-website-to-test.js <path-to-html>");
    process.exit(2);
  }
